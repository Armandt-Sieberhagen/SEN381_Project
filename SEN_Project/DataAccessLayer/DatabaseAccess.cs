    ...
    public class DatabaseAccess
    {
        ...
        public void Add(Address _Address)
        {
            ...
            if (_Add == null)
            {
                ...
                DatabaseController.current.ExecuteCommand(Command);
                PresentationController.current.ShowSuccess("Address has been added");
            }
            else
            {
                PresentationController.current.ShowError("Database already contains this record");
            }
        }
        public void Add(Call _Call)
        {
            if (SearchIndex(_Call) != -1)
            {
                PresentationController.current.ShowError("Call is already in database");
                return;
            }
            string Command = "INSERT INTO tbl_Calls (StartTime,EndTime) VALUES ('" + _Call.StartTime + "','" + _Call.EndTime + "')";
            if (DatabaseController.current.ExecuteCommand(Command))
            {
                PresentationController.current.ShowSuccess("Call has been added!");
            }
            else
            {
                PresentationController.current.ShowError("Call could not be added");
            }
        }
